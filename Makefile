...
html: media style templates/cv.html parts $(SRC_DIR)/cv.md | directories
	pandoc --standalone \
	  ...
	  --output $(DIST_DIR)/IlyaYaroshenkoCV.html $(SRC_DIR)/cv.md

# Target for building CV document in PDF
pdf: html pdftags
ifeq ($(HTMLTOPDF),wkpdf)
	wkpdf --paper a4 --margins 30 --print-background yes --orientation portrait --stylesheet-media print --source $(DIST_DIR)/IlyaYaroshenkoCV.pdf.html --output $(DIST_DIR)/IlyaYaroshenkoCV.pdf
else
